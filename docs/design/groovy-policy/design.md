...
`policy-only` 下，Server bootstrap 跳过安装 HugeSecurityManager，但保留原有 DNS 安全属性检查。若 JVM 已由其他启动参数或 agent 安装 SecurityManager，则启动失败，不尝试卸载。非法模式也会启动失败。所有模式继续使用已有身份认证和图权限检查。Store 公共 RPC 组件的取消与终止处理修复适用于全部模式，legacy 关闭的是新增脚本规则。
...
| 内容 | 策略行为 |
|---|---|
| 方法调用 | 按声明类型、方法名和参数类型匹配固定清单，不按包名前缀放行；静态 import 和别名调用也接受相同检查 |
...
各 profile 及会话事务扩展的 `*-methods.txt` 资源文件是该版本的方法签名清单。启动创建策略时会校验依赖中实际签名与清单是否一致。依赖新增或删除重载时，不能自动扩展脚本权限，必须随代码显式更新清单和测试。
...
| 每次执行 | 新建 Script 和独立 Binding，使用当前入口提供的图及用户上下文；Session 在成功完成请求后保留受检数据 |
| 缓存上限 | 每个引擎 256 项，空闲 30 分钟过期；Session 引擎随各自会话关闭，其他入口按 profile 复用实例 |
| 预编译条件 | Store 复用编译结果，逐记录执行时仍检查绑定类型，不重复构造源码缓存键 |
| 关闭 | 关闭引擎后拒绝执行，清理缓存和类加载器；编译线程空闲 30 秒后退出 |

结果不能携带 Class、闭包、引擎上下文、图对象或遍历源，也会检查集合、Map、Optional、Path 和属性值中的嵌套对象。返回未执行的遍历时，在末尾增加结果检查步骤，避免把执行对象藏在 `g.inject(...)` 的惰性结果中。`.iterate()` 或已经部分消费的遍历会包装剩余结果并继续检查每个元素，关闭时转发到原遍历。允许的方法返回普通 Iterator 时，也按原协议逐项输出并校验，消费结束或提前关闭会释放原迭代器；嵌套在数据容器中的 Iterator 仍不作为普通数据接受。

Session ID 必须是非空字符串，长度不超过 256 个字符；会话绑定创建它的认证身份和连接。Session 保留 Groovy 的变量区分。`x = 1` 创建的绑定可以在后续请求中使用；`def x = 1` 或 `int x = 1` 声明的局部变量仅属于当前脚本。保留值仍受数据类型和大小限制，不支持把图、遍历、闭包或服务对象存入会话状态。请求使用数据副本，惰性结果迭代期间的变量和集合修改也要通过检查，成功完成后才保存到会话；失败或超时不会留下这些数据修改。这一规则可能改变原脚本依赖失败请求部分修改集合的行为，图事务仍按 TinkerPop 的会话事务规则处理。图别名只保留名称，每次请求重新从 GraphManager 解析，避免图删除或替换后继续使用旧对象。Session 不重新执行部署者的启动脚本。请求进入父类调度前，会在当前会话锁内重新核对注册对象与接收状态，避免超时删除与二次查找之间重建原生会话。

会话沿用 TinkerPop 的单线程事务执行和关闭流程，原生事务提交、回滚请求进入原有处理路径。Session 脚本也可以使用 `g.tx().commit()`、`g.tx().rollback()` 或图别名上的对应调用。独立会话方法清单仅增加获取事务及提交、回滚的四个精确签名，不开放线程化事务和事务回调；事务对象不能作为结果导出或存入会话。旧客户端的 close 操作、连接断开和会话超时继续采用 TinkerPop 3.8.1 各自的关闭语义。HTTP 标准 Gremlin endpoint 没有会话执行链路，序列化 HTTP 请求不能借 session 字段绕过这一限制。
...
| WebSocket Session 请求 | 原行为 | 每会话独立受限引擎，保留允许的数据变量和原事务流程；会话绑定认证身份与连接 |
| Cypher 请求 | 原行为 | 保留已有 Cypher processor 的解析与遍历执行路径，不改为 Groovy eval |
...
Store 条件使用只读元素视图。`element.id()` 返回元素 ID 字符串，`element.label()` 返回标签名，`element.property('age')` 按属性名取值。Store 通过已有的图 Schema 读取组件还原二进制记录中的名称，不要求调用者把过滤表达式改写为内部 ID。缺失属性返回 `null`，可以使用 `element.property('age') != null` 或 `element.properties().containsKey('age')` 判断是否存在；缺失属性本身不会终止扫描。Schema 读取、数据解码或脚本求值失败仍会终止扫描并传播错误，非 Boolean 返回值也会使扫描失败。日期转换为毫秒值，Blob 转换为字节数据，LIST / SET 属性中的日期和 Blob 同样转换。多值属性按数据列表提供，LIST 保留顺序和重复项，SET 保留去重后的值，不承诺迭代顺序。视图不暴露原始 BaseElement、BaseProperty 或可变 Schema 对象。
...
编译和运行错误不会自动转到旧引擎重试。新引擎的异常不携带完整脚本和绑定值；原 Gremlin Server 请求日志仍由原日志配置控制。Store 的读取与发送任务先发布取消句柄，再提交执行，保证跨批次调度后取消仍能中断当前任务。流式响应已经发出的记录无法收回，调用方必须以最终错误判断整次扫描失败。Schema、任务调度等已提交操作不承诺跨服务原子回滚。
