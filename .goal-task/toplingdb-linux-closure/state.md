...
| P1 历史标准集群补证 | 未开始 | 1+1+1 生命周期、3+3+3 功能与 HA 有日志和业务证据；结果绑定 `9aba`，不算当前 SHA |
...
goal 启动后的第一动作：只对 `hg-closure-standard-111` 做标准 RocksDB 停止、重启和已确认写入复查。不部署 Topling，不改动 `hg-closure-standard-333`。
